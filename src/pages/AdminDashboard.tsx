...
import { AlertTriangle, Globe, Users, MapPin, Clock, Scale, HelpCircle, ImageIcon, Mail, Settings, Eye, MessageSquare, Newspaper, Share2, FileWarning } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [pendingSocialsCount, setPendingSocialsCount] = useState(0);
  ...
  const fetchPendingSocials = async () => {
    try {
      const { count, error } = await supabase
        .from('social_posts')
        .select('*', { count: 'exact', head: true })
      ...
      if (error) throw error;
      setPendingSocialsCount(count || 0);
    } catch (error) {
      console.error('Error fetching pending social posts count:', error);
    }
  };
      ...
      <div className="min-h-screen bg-background py-8">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-8">
            ...
            <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate("/admin/socials")}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Social Posts</CardTitle>
                <Share2 className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">{pendingSocialsCount}</div>
                <p className="text-xs text-muted-foreground mt-1">Pending approval</p>
              </CardContent>
            </Card>
            ...
            <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate("/admin/complaints")}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Complaints Database</CardTitle>
                <FileWarning className="h-4 w-4 text-muted-foreground" />
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold">Manage</div>
                <p className="text-xs text-muted-foreground mt-1">Add complaints</p>
              </CardContent>
            </Card>
